...
for ii in range(0,d1-1000,1):
    ## Extracting array dimensions for easier calculations and cleaner code
    frame_med_norm[ii] = np.median(tdata[ii,:,:])-np.median(tdata[ii+1000,:,:])
    if ii%10000==0:
...
for frame_set in range(plusminus,d1-plusminus,1):
    frame_reduced[frame_set] = np.median(frame_med_norm[frame_set-plusminus:frame_set+plusminus-1])
    if frame_set%10000==0:
        print("\tframe_set:\t{} / {}".format(frame_set,d1-plusminus))
plt.clf()
for ii in range(0,nsubplots,1):
    ax = plt.subplot(gs[ii,0])
    plt.plot(frame_reduced[int(ii*d1/nsubplots):int((ii+1)*(d1/nsubplots))],linewidth=0.3,c='k')
    plt.xlim(left=0,right=d1/nsubplots)
    if ii != nsubplots-1:
        ax.xaxis.set_visible(False)
plt.subplots_adjust(hspace=0)

plt.savefig('reddata.pdf')
plt.show()
