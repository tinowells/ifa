...
dfile='180921_FrequencyResponse_64x256_2Vbias_10000f_cube.fits'
nsubplots = 4
...
## (10000,256,64) - extract last :32 and plot

#plt.clf()
#for ii in range(0,d1,1):
#    plt.plot(xidx[ii],np.median(tdata[ii,:,:]),'.',c='r')
#    if ii%100==0:
#        print("ii == {} \t\t {}".format(ii,np.median(tdata[ii,:,:])))
#plt.show()
#pdb.set_trace()

for ii in range(0,d1-10,1):
    ## Extracting array dimensions for easier calculations and cleaner code
    frame_med_norm[ii] = np.median(tdata[ii,:,:])-np.median(tdata[ii+10,:,:])
    ## plot raw values
    if ii%1000==0:
...
#    plt.plot(frame_med_norm[int(ii*d1/nsubplots):int((ii+1)*(d1/nsubplots))],linewidth=0.3,c='k')
#    plt.xlim(left=0,right=d1/nsubplots)
#    if ii != nsubplots-1:
#        ax.xaxis.set_visible(False)
#plt.subplots_adjust(hspace=0)
#plt.savefig('normdata.pdf')
#plt.show()
...
plt.subplots_adjust(hspace=0)
plt.legend()
plt.savefig('nord_reduced_data.pdf')
plt.show()
