...
def check_peakidx(data,peaks):
    ...
    peak_idx.append(peaks[-1]) ## doing diff excludes last peak, which is important and useful, this is here to include it
...
def plot_data(data,offidx,ridx):
    ...
    ## plotting ramps as indicvidual subplots --- REPEATED PLOT WITH TIME CONSTANT< EXCLUDING
    #plt.clf()
    #plt.ylabel(ylbl)
...
if __name__=='__main__':
    ...
    datatemp = input("\tEnter temperature corresponding to dataset (e.g., 60, 75, 90): ") ## CHANGE THIS TO PICK OUT FROM FITS FILE NAME
    ...
    timeconstant_onoff = tcf.calc_timeconstant(reduced_data,ramp_idx,offsetidx,ramp_offset,datatemp,flipdata=True)
    timeconstant_offon = tcf.calc_timeconstant(reduced_data,ramp_idx,offsetidx,ramp_offset,datatemp)
